...
const FALLBACK = "/products/broccoli-500gm.png";
    ...
    <div style={{ fontFamily: "'Inter','Poppins',-apple-system,sans-serif", background: "#f4f6f0", minHeight: "100vh" }}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .p-ticker-wrap{overflow:hidden;background:#1a3c2e;}
        .p-ticker-desktop{display:flex;justify-content:center;align-items:center;flex-wrap:nowrap;gap:0;padding:7px 1rem;overflow:hidden;}
        .p-ticker-mobile{display:none;position:fixed;top:0;left:0;right:0;z-index:198;width:100%;background:#1a3c2e;border-bottom:1px solid #174123;}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "1.5rem" }}>
        {loading ? (
          ...
          <>
            ...
            <div className="p-grid">
              {paginated.map(p => (
                <div key={p._id} className="p-card">
                  <img
                    src={p.imageUrl || `/products/${p.slug}.png`}
                    alt={p.name}
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", marginTop: "2rem", flexWrap: "wrap" }}>
                <button
                  onClick={() => setPage(p => Math.max(1, p - 1))}
                  disabled={safePage === 1}
                  style={{ padding: "8px 18px", borderRadius: "8px", border: "1.5px solid #e5e7eb", background: safePage === 1 ? "#f9fafb" : "#fff", color: safePage === 1 ? "#d1d5db" : "#374151", fontWeight: 700, fontSize: "13px", cursor: safePage === 1 ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
                  ← Prev
                </button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                  <button
                    key={n}
                    onClick={() => setPage(n)}
                    style={{ width: "36px", height: "36px", borderRadius: "8px", border: "1.5px solid", borderColor: safePage === n ? "#2d8a4e" : "#e5e7eb", background: safePage === n ? "#2d8a4e" : "#fff", color: safePage === n ? "#fff" : "#374151", fontWeight: 700, fontSize: "13px", cursor: "pointer", fontFamily: "inherit" }}>
                    {n}
                  </button>
                ))}
                <button
                  ...
                  style={{ padding: "8px 18px", borderRadius: "8px", border: "1.5px solid #e5e7eb", background: safePage === totalPages ? "#f9fafb" : "#fff", color: safePage === totalPages ? "#d1d5db" : "#374151", fontWeight: 700, fontSize: "13px", cursor: safePage === totalPages ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
                  Next →
                </button>
              </div>
            )}
